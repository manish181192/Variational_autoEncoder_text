...
class VariationalAutoencoder(object):
    ...
    def __init__(self, network_architecture, transfer_fct=tf.nn.softplus,
        ...
        # tf Graph input
        self.ip = tf.placeholder(tf.float32, [None, network_architecture["ip_size"]])

        self.x = self.get_lstm_states(self.ip)
    ...
    def get_context_vector(self, x):
        ...
        with tf.name_scope('relation_vector'):
            ...
            #                                hidden_dim= self.Da)
        self.outputs = tf.concat(values=self.outputs, concat_dim=1)
        return self.outputs
    def get_lstm_states(self, ip):

        ip = tf.reshape(ip, [-1, 14, 300])
        self.dropout_keep_prob = tf.constant(0.5)

        self.x_left = tf.unstack(ip, 14, 1)
        self.Vc_left = self.get_context_vector(self.x_left)
        return self.Vc_left
